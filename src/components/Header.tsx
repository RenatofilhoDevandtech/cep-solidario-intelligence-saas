...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 apple-liquid-glass border-b border-slate-200/70 transition-colors">
      ...
      <div className="bg-slate-900/95 text-slate-200 text-[11px] py-1 px-3 sm:px-4 border-b border-slate-800/80">
      ...
      <div className="header-main max-w-7xl mx-auto px-3 sm:px-6 flex items-center justify-between gap-3 sm:gap-4">
        {/* Brand Identity */}
        <div
          onClick={() => setActiveTab('search')}
          className="header-brand flex items-center gap-2.5 sm:gap-3 cursor-pointer select-none group"
        >
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-linear-to-br from-slate-950 via-slate-900 to-slate-800 text-white flex items-center justify-center shadow-sm ring-1 ring-black/5 group-hover:scale-105 transition-transform duration-200">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider bg-slate-100/90 text-slate-700 px-2 py-0.5 rounded-full border border-slate-200/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
                Pro
              </span>
            </div>
        ...
        <nav
          className="hidden lg:flex items-center bg-slate-200/60 p-1 rounded-2xl backdrop-blur-md border border-slate-300/40 shadow-inner"
        ...
        <div className="header-actions flex items-center gap-2.5">
          {loggedCompany ? (
            <div className="flex items-center gap-2 bg-slate-100/90 border border-slate-200/80 rounded-full pl-3 pr-1.5 py-1 shadow-xs">
              <Building2 className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
              <span className="text-xs font-bold text-slate-900 truncate max-w-[110px] sm:max-w-[140px]">
                {loggedCompany.name}
              </span>
              <span className="text-[10px] text-indigo-800 font-bold bg-indigo-100/80 px-2 py-0.5 rounded-full">
                {loggedCompany.plan}
              </span>
              <button
          ...
          <button
            onClick={openPricing}
            className="flex items-center gap-1.5 bg-slate-900 hover:bg-slate-800 active:scale-95 text-white px-4 py-2 rounded-full text-xs font-bold shadow-sm transition-all"
          >
